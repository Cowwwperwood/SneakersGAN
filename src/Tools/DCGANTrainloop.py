def init_wandb():
    wandb.finish()
    wandb.init(project='DeepGan', config={
        "lr": 0.0002,
        "b1": 0.5,
        "b2": 0.999,
        "epochs": 100,
...
def lsgan_loss(y_hat, y):
    return 0.5 * nn.MSELoss()(y_hat, y)

def train_generator(generator, discriminator, imgs, z, optimizer_g, device):
    valid = torch.ones(imgs.size(0), 1).to(device)
    generated_imgs = generator(z)
    g_loss = lsgan_loss(discriminator(generated_imgs), valid)
...
def train_discriminator(discriminator, generator, imgs, z, optimizer_d, device):
    ...
    real_loss = lsgan_loss(discriminator(imgs), valid)
    fake_loss = lsgan_loss(discriminator(generator(z).detach()), fake)
...
def setup_and_train():
    ...
    generator = DCGenerator(noise_dim=NOISE_DIM, img_shape=IMAGE_SIZE + (3,)).to(device) 
    discriminator = DCDiscriminator(img_shape=IMAGE_SIZE + (3,)).to(device) 
